 ...
 * اختبارات رياضية للتحقق من صحة المحرك
 */

import { DominoGameEngine } from './game-engine';
...
import { calculateSnakeLayout, verifyLayout, DEFAULT_LAYOUT } from './layout';
...
function testBoardPlaySequence(): boolean {
  ...
  const t1 = createTile(3 as any, 5 as any);
  ...
  const t2 = createTile(5 as any, 2 as any);
  ...
  const t3 = createTile(1 as any, 3 as any);
...
function testBlockedGame(): boolean {
  const board = BoardManager.createEmpty();
  board.playTile(createTile(1 as any, 6 as any), 'left');
  ...
  const bad = createTile(2 as any, 3 as any);
  if (board.canPlay(bad)) {
    console.error('FAIL: Should not play [2|3] on 1,6');
    return false;
  }

  const good = createTile(1 as any, 2 as any);
...
function testDoubleRotation(): boolean {
  const double = createTile(4 as any, 4 as any);
